...
const WellTable = () => {
    ...
    const fetchData = async (page, pageSize, ngdu, workshop) => {
        setLoading(true);
        try {
            let url = `/api/well/table/nag?page=${page}&pageSize=${pageSize}`;
    ...
    const columns = [
        {
            ...
            render: (text, record) => (
                <a
                    onClick={() => navigate(`/Mapp_nag?wellId=${record.idWell}&longitude=${record.longitude}&latitude=${record.latitude}`)}
